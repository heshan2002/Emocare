...
export default function Neutral() {
  ...
  return (
    <MoodLayout
      mood="neutral"
      title="You feel Calm"
      subtitle={`Good. Let’s keep a healthy routine`}
      primaryActions={[
        { label: "🎮 Simple Game", onClick: () => show("Opening a simple game…") },
        { label: "💬 Chat with Bot", onClick: () => show("Opening chat…") },
      ]}
      secondaryActions={[
        { label: "🚶 Short Walk Reminder", onClick: () => show("Walk reminder set…") },
        { label: "💧 Drink Water", onClick: () => show("Water reminder set…") },
      ]}
    >
      {toast && (
        <div style={toastStyle("#11B6F2")}>
